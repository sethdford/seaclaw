...
#if SC_IS_TEST
sc_error_t sc_twitter_test_inject_mock(sc_channel_t *ch, const char *session_key,
...
const char *sc_twitter_test_get_last_message(sc_channel_t *ch, size_t *out_len) {
    if (!ch || !ch->ctx)
        return NULL;
    sc_twitter_ctx_t *c = (sc_twitter_ctx_t *)ch->ctx;
    if (out_len)
