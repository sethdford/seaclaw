...
static void test_hula_call_with_idempotency_dedup(void) {
    ...
    /* Create program: CALL echo with args */
    ...
    /* Ensure tool_name and args_json are set */
    ...
    /* Create and attach idempotency registry */
    ...
    HU_ASSERT_EQ(stats1.entry_count, 1); /* Tool result should be cached */
    HU_ASSERT_EQ(stats1.total_hits, 0);  /* No hits yet, just recording */
    HU_ASSERT_EQ(stats1.total_misses, 1); /* One miss (cache check before recording) */
    ...
    HU_ASSERT_STR_EQ(result2->output, result1->output); /* Results should match */

    hu_idempotency_stats(reg, &stats1);
    HU_ASSERT_EQ(stats1.total_hits, 1);   /* One cache hit */
    HU_ASSERT_EQ(stats1.total_misses, 0);
...
static void test_hula_seq_with_idempotency(void) {
    ...
    /* Create program: SEQ { CALL echo("msg1"), CALL echo("msg2") } */
    hu_hula_program_t prog;
    ...
    HU_ASSERT_EQ(stats.entry_count, 2); /* Both tool calls cached */
    ...
    HU_ASSERT_STR_EQ(r2_1->output, r1_1->output);
    HU_ASSERT_STR_EQ(r2_2->output, r1_2->output);

    hu_idempotency_stats(reg, &stats);
    HU_ASSERT_EQ(stats.total_hits, 2); /* Two cache hits */
...
void run_idempotency_hula_integration_tests(void) {
    ...
    /* Note: HuLa integration tests require full HuLa initialization.
     * For now, we focus on unit tests. Integration tests in production
     * will verify the interaction between HuLa and idempotency_registry. */
