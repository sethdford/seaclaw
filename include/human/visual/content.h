...
/* Screenshot: test → "/tmp/mock-screenshot.png"; else macOS → temp PNG via computer_use (policy
 * required). */
...
/* Query the local Apple Photos SQLite database for recent photos.
 * photos_db_path: ~/Pictures/Photos Library.photoslibrary/database/Photos.sqlite
 * Returns up to max_results entries. Caller frees *out via hu_visual_entries_free. */
#ifdef HU_ENABLE_SQLITE
...
/* Build the default Apple Photos database path for the current user.
 * Writes into buf, returns length. macOS-only. */
size_t hu_visual_apple_photos_db_path(char *buf, size_t cap);

#endif /* HU_VISUAL_CONTENT_H */
