...
- **`hu_hula_trace_persist`** — write a trace file (CLI, agent, and compiler paths embed a **`program`** snapshot via `hu_hula_to_json` when possible so **`hula replay`** works on `~/.human/hula_traces/*.json`). Callers may set **`program_source`** (optional text: model JSON, tag-inner JSON, or IR audit JSON for tool-call paths).
