...
static sc_error_t split_path(sc_allocator_t *alloc, const char *path, char ***out_tokens,
...
static sc_json_value_t *value_at_path(const sc_json_value_t *root, char **tokens,
                                      size_t token_count) {
    const sc_json_value_t *cur = root;
    for (size_t i = 0; i < token_count; i++) {
        if (!cur || cur->type != SC_JSON_OBJECT)
            return NULL;
 ...
 * segment). */
static sc_json_value_t *ensure_and_walk(sc_allocator_t *alloc, sc_json_value_t *root, char **tokens,
                                        size_t token_count) {
    if (token_count == 0)
        return root;
...
static sc_error_t write_config_file(sc_allocator_t *alloc, const char *path, const char *content,
                                    size_t content_len) {
...
static sc_error_t read_config_file(sc_allocator_t *alloc, const char *path, char **out_content,
...
sc_error_t sc_config_mutator_get_path_value_json(sc_allocator_t *alloc, const char *path,
                                                 char **out_json) {
...
sc_error_t sc_config_mutator_mutate(sc_allocator_t *alloc, sc_mutation_action_t action,
                                    const char *path, const char *value_raw,
                                    sc_mutation_options_t options, sc_mutation_result_t *out) {
    if (!alloc || !path || !out)
        return SC_ERR_INVALID_ARGUMENT;
