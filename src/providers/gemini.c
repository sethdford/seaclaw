    ...
    const char *data, size_t data_len, void *userdata)
{
    ...
    if (s->content_len + dlen + 1 > s->content_cap) {
        ...
        while (new_cap < s->content_len + dlen + 1) new_cap *= 2;
...
static size_t gemini_stream_write_cb(const char *chunk, size_t chunk_len, void *userdata) {
    gemini_stream_ctx_t *s = (gemini_stream_ctx_t *)userdata;
    if (s->last_error != SC_OK) return chunk_len;
    sc_error_t err = sc_sse_parser_feed(&s->parser, chunk, chunk_len,
        gemini_sse_event_cb, userdata);
    ...
    sc_chat_response_t *out)
{
    ...
    if (!gc || !request || !out) return SC_ERR_INVALID_ARGUMENT;
    (void)model;
    (void)model_len;
    (void)temperature;
    ...
    if (system_prompt && system_len > 0) {
        ...
        if (!si_parts) { sc_json_free(alloc, root); return SC_ERR_OUT_OF_MEMORY; }
        sc_json_value_t *si_obj = sc_json_object_new(alloc);
        if (!si_obj) { sc_json_free(alloc, si_parts); sc_json_free(alloc, root); return SC_ERR_OUT_OF_MEMORY; }
        sc_json_object_set(alloc, si_obj, "text", sc_json_string_new(alloc, system_prompt, system_len));
        sc_json_array_push(alloc, si_parts, si_obj);
        sc_json_value_t *si = sc_json_object_new(alloc);
        if (!si) { sc_json_free(alloc, si_parts); sc_json_free(alloc, root); return SC_ERR_OUT_OF_MEMORY; }
    ...
    if (!contents) { sc_json_free(alloc, root); return SC_ERR_OUT_OF_MEMORY; }
    ...
    for (size_t i = 0; i < request->messages_count; i++) {
        ...
        if (!part) { sc_json_free(alloc, root); return SC_ERR_OUT_OF_MEMORY; }
        const char *role_str = "user";
        if (m->role == SC_ROLE_ASSISTANT) role_str = "model";
        sc_json_object_set(alloc, part, "role", sc_json_string_new(alloc, role_str, strlen(role_str)));
        sc_json_value_t *parts_arr = sc_json_array_new(alloc);
        if (!parts_arr) { sc_json_free(alloc, part); sc_json_free(alloc, root); return SC_ERR_OUT_OF_MEMORY; }
        if (m->role == SC_ROLE_TOOL && m->tool_call_id && m->name && m->content) {
            ...
            if (fr) {
                ...
                if (fresp) {
                    sc_json_object_set(alloc, fresp, "name", sc_json_string_new(alloc, m->name, m->name_len));
                    sc_json_object_set(alloc, fresp, "response", sc_json_string_new(alloc, m->content, m->content_len));
                }
            ...
            for (size_t k = 0; k < m->tool_calls_count; k++) {
                ...
                if (fc) {
                    ...
                    if (fc_inner && tc->name && tc->name_len > 0) {
                        sc_json_object_set(alloc, fc_inner, "name", sc_json_string_new(alloc, tc->name, tc->name_len));
                        if (tc->arguments && tc->arguments_len > 0) {
                            sc_json_value_t *args_val = NULL;
                            if (sc_json_parse(alloc, tc->arguments, tc->arguments_len, &args_val) == SC_OK)
            ...
            if (text_part) {
                sc_json_object_set(alloc, text_part, "text", sc_json_string_new(alloc, m->content, m->content_len));
    ...
    if (request->tools && request->tools_count > 0) {
        ...
        if (func_decls) {
            for (size_t i = 0; i < request->tools_count; i++) {
                ...
                if (!decl) { sc_json_free(alloc, func_decls); sc_json_free(alloc, root); return SC_ERR_OUT_OF_MEMORY; }
                sc_json_object_set(alloc, decl, "name", sc_json_string_new(alloc, request->tools[i].name, request->tools[i].name_len));
                sc_json_object_set(alloc, decl, "description", sc_json_string_new(alloc,
    ...
    sc_json_object_set(alloc, root, "generationConfig",
        sc_json_object_new(alloc));
    ...
    if (gen_cfg) {
        ...
        sc_json_object_set(alloc, gen_cfg, "maxOutputTokens", sc_json_number_new(alloc, (double)max_tok));
    }
    ...
    if (gc->oauth_token && gc->oauth_token_len > 0) {
        int na = snprintf(auth_buf, sizeof(auth_buf), "Bearer %.*s",
            (int)gc->oauth_token_len, gc->oauth_token);
        ...
        int n = snprintf(url_buf, sizeof(url_buf),
            "%.*s/%.*s:generateContent",
            (int)SC_GEMINI_BASE_LEN, SC_GEMINI_BASE,
            (int)model_len, model);
    ...
    if (hresp.status_code < 200 || hresp.status_code >= 300) {
        ...
        if (hresp.status_code == 401 || hresp.status_code == 403) return SC_ERR_PROVIDER_AUTH;
                    ...
                    if (fc && fc->type == SC_JSON_OBJECT) {
                        ...
                        if (!tcs) {
                            tcs = (sc_tool_call_t *)alloc->alloc(alloc->ctx,
                                parts->data.array.len * sizeof(sc_tool_call_t));
                            if (!tcs) break;
    ...
    if (usage && usage->type == SC_JSON_OBJECT) {
        ...
        out->usage.completion_tokens = (uint32_t)sc_json_get_number(usage, "candidatesTokenCount", 0);
...
static bool gemini_supports_native_tools(void *ctx) { (void)ctx; return true; }
static bool gemini_supports_streaming(void *ctx) { (void)ctx; return true; }
static bool gemini_supports_vision(void *ctx) { (void)ctx; return true; }
static bool gemini_supports_vision_for_model(void *ctx, const char *model, size_t model_len) {
    (void)ctx; (void)model; (void)model_len;
    return true;
}
static const char *gemini_get_name(void *ctx) { (void)ctx; return "gemini"; }
...
{
    sc_gemini_ctx_t *gc = (sc_gemini_ctx_t *)ctx;
    if (!gc || !request || !callback || !out) return SC_ERR_INVALID_ARGUMENT;
    ...
    if (gc->oauth_token && gc->oauth_token_len > 0) {
        int na = snprintf(auth_buf, sizeof(auth_buf), "Bearer %.*s",
            (int)gc->oauth_token_len, gc->oauth_token);
        if (na > 0 && (size_t)na < sizeof(auth_buf)) auth_header = auth_buf;
    ...
    err = sc_http_post_json_stream(alloc, url_buf, auth_header, NULL,
        body, body_len, gemini_stream_write_cb, &sctx);
...
static void gemini_deinit(void *ctx, sc_allocator_t *alloc) {
    ...
    sc_gemini_ctx_t *gc = (sc_gemini_ctx_t *)ctx;
    if (!gc) return;
    if (gc->api_key) free(gc->api_key);
    if (gc->oauth_token) free(gc->oauth_token);
    free(gc);
}
    ...
    sc_provider_t *out)
{
    ...
    if (!gc) return SC_ERR_OUT_OF_MEMORY;
    if (api_key && api_key_len > 0) {
        gc->api_key = (char *)malloc(api_key_len + 1);
        if (!gc->api_key) { free(gc); return SC_ERR_OUT_OF_MEMORY; }
    ...
    sc_provider_t *out)
{
    ...
    (void)base_url;
    (void)base_url_len;
    if (!oauth_token || oauth_token_len == 0) return SC_ERR_INVALID_ARGUMENT;
    sc_gemini_ctx_t *gc = (sc_gemini_ctx_t *)calloc(1, sizeof(*gc));
    if (!gc) return SC_ERR_OUT_OF_MEMORY;
    gc->oauth_token = (char *)malloc(oauth_token_len + 1);
    if (!gc->oauth_token) { free(gc); return SC_ERR_OUT_OF_MEMORY; }
