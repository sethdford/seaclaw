...
- **`hu_hula_trace_persist`**: writes a JSON file under `trace_dir` or `~/.human/hula_traces/` (POSIX). In `HU_IS_TEST`, passing `NULL` for `trace_dir` is a no-op; tests pass an explicit temp directory.
