...
static sc_error_t diff_execute(void *ctx, sc_allocator_t *alloc,
    const sc_json_value_t *args, sc_tool_result_t *out)
{
    ...
    if (strcmp(action, "diff") == 0) {
        ...
        if (!fa || !fb) { *out = sc_tool_result_fail("missing file_a or file_b", 23); return SC_OK; }
#if SC_IS_TEST
        ...
        *out = sc_tool_result_ok_owned(msg, msg ? strlen(msg) : 0);
        ...
        fseek(af, 0, SEEK_END); long alen = ftell(af); fseek(af, 0, SEEK_SET);
        fseek(bf, 0, SEEK_END); long blen = ftell(bf); fseek(bf, 0, SEEK_SET);
        ...
        if (!abuf || !bbuf) {
            if (abuf) alloc->free(alloc->ctx, abuf, (size_t)alen + 1);
            if (bbuf) alloc->free(alloc->ctx, bbuf, (size_t)blen + 1);
            fclose(af); fclose(bf);
            *out = sc_tool_result_fail("oom", 3); return SC_OK;
        }
        fread(abuf, 1, (size_t)alen, af); abuf[alen] = '\0';
        fread(bbuf, 1, (size_t)blen, bf); bbuf[blen] = '\0';
        fclose(af); fclose(bf);
...
static const char *diff_name(void *ctx) { (void)ctx; return TOOL_NAME; }
static const char *diff_desc(void *ctx) { (void)ctx; return TOOL_DESC; }
static const char *diff_params(void *ctx) { (void)ctx; return TOOL_PARAMS; }
