...
#define SC_OPENROUTER_URL "https://openrouter.ai/api/v1/chat/completions"

typedef struct sc_openrouter_ctx {
    char *api_key;
    size_t api_key_len;
} sc_openrouter_ctx_t;

static sc_error_t openrouter_chat(void *ctx, sc_allocator_t *alloc,
                                  const sc_chat_request_t *request, const char *model,
                                  size_t model_len, double temperature, sc_chat_response_t *out);

static sc_error_t openrouter_chat_with_system(void *ctx, sc_allocator_t *alloc,
    ...
    sc_error_t err = openrouter_chat(ctx, alloc, &req, model, model_len, temperature, &resp);
    if (err != SC_OK)
        return err;
...
static sc_error_t openrouter_chat(void *ctx, sc_allocator_t *alloc,
    ...
    if (request->tools && request->tools_count > 0) {
        ...
        if (tcs) {
            ...
            tcs[0].id = sc_strndup(alloc, "call_mock_or", 12);
            tcs[0].id_len = 12;
    ...
    err = sc_http_post_json(alloc, SC_OPENROUTER_URL, auth_buf, body, body_len, &hresp);
    alloc->free(alloc->ctx, body, body_len);
    if (err != SC_OK)
        return err;
...
static sc_error_t openrouter_stream_chat(void *ctx, sc_allocator_t *alloc,
                                         const sc_chat_request_t *request, const char *model,
