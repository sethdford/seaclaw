...
test.describe("Turing (Demo)", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/?demo#turing");
    await waitForViewReady(page, "hu-turing-view");
  });
  ...
  test("view renders with substantive content (not blank)", async ({ page }) => {
    await expect(async () => {
      const text = await page.evaluate(deepText("hu-turing-view"));
  ...
  test("shows page hero and Turing heading", async ({ page }) => {
    await expect(async () => {
      expect(await page.evaluate(shadowExists("hu-turing-view", "hu-page-hero"))).toBe(true);
      const text = await page.evaluate(deepText("hu-turing-view"));
