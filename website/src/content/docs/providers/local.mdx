...
SeaClaw supports multiple local inference servers. **No API key is required** for any local provider. Add your provider to `~/.seaclaw/config.json` and set `default_provider` and `default_model`.
