...
_human() {
    ...
    if [[ $COMP_CWORD -eq 1 ]]; then
        COMPREPLY=($(compgen -W '--version -v --help -h --mcp agent agents auth calibrate capabilities channel config cron doctor eval feed gateway hardware help hula init mcp memory migrate ml models onboard paperclip persona plugins pwa research sandbox service service-loop setup skills status update version workspace' -- "$cur"))
        return
    fi
