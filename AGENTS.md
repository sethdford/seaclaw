...
- minimal binary size (~13148 KB release with LTO)
...
Current scale: **1224 source + header files, ~280K lines of C, ~117K lines of tests, 6854 tests, 38 channels**.

Performance baseline (macOS aarch64, MinSizeRel+LTO):

| Metric                   | Measured       |
| ------------------------ | -------------- |
| Binary size              | ~13148 KB       |
| Text section             | 480 KB         |
...
2. **Binary size and memory are hard product constraints**
   ...
   - Current release binary: ~13148 KB (all features with LTO).
...
5. **All 6854+ tests must pass at zero ASan errors**
