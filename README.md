...
<p align="center">
  ...
  <strong>~13148 KB binary. < 6 MB RAM. Boots in <30 ms. Runs on anything with a CPU.</strong>
</p>
...
```
~13148 KB binary · <30 ms startup · 6854+ tests · 97 providers · 38 channels (37 production + 1 experimental) · 85 tools · Pluggable everything
```

### Features

- **Impossibly Small:** ~13148 KB static binary — no runtime, no VM, no framework overhead.
...
| **RAM** ¹         | —                                                | —                                           | < 10 MB                                        | < 5 MB                                                | **< 6 MB**        |
| **Binary Size** ¹ | ~28 MB (npm dist)                                | N/A (Python)                                | ~8 MB                                          | ~8.8 MB                                               | **~13148 KB**      |
...
```
Binary size:   ~13148 KB (MinSizeRel + LTO, all channels)
...
**Integration tests** (optional): real libcurl HTTP and SQLite file I/O (`human_core` without `HU_IS_TEST`):

```bash
cmake --preset integration && cmake --build --preset integration
./build-integration/human_integration_tests
```

Release build (~13148 KB):
...
Test files: 334
Tests: 6374+
Binary: ~13148 KB (MinSizeRel + LTO, all channels)
Peak RSS: ~5.7 MB
...
tests/ 334 test files, 6854 tests
