...
static const char *prefix_for_type(hu_benchmark_type_t type) {
    switch (type) {
        case HU_BENCHMARK_GAIA: return "gaia-";
        case HU_BENCHMARK_SWE_BENCH: return "swe-bench-";
        case HU_BENCHMARK_TOOL_USE: return "tool-use-";
    }
    return "";
}

hu_error_t hu_benchmark_load(hu_allocator_t *alloc, hu_benchmark_type_t type,
                             const char *json, size_t json_len, hu_eval_suite_t *out) {
    ...
    if (!new_name) return HU_ERR_OUT_OF_MEMORY;
    memcpy(new_name, prefix, prefix_len);
    memcpy(new_name + prefix_len, orig, orig_len + 1);

    if (out->name) alloc->free(alloc->ctx, out->name, orig_len + 1);
    out->name = new_name;
    return HU_OK;
}
