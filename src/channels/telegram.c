...
static const char TELEGRAM_COMMANDS_HELP[] =
    ...
    size_t cursor, size_t *out_end)
{
    ...
    if (max_chunk > TELEGRAM_MAX_MSG) max_chunk = TELEGRAM_MAX_MSG;
...
{
    if (!c->allow_from || c->allow_from_count == 0) return true;
    for (size_t i = 0; i < c->allow_from_count; i++) {
        const char *a = c->allow_from[i];
        if (!a) continue;
        if (strcmp(a, "*") == 0) return true;
        size_t alen = strlen(a);
        if (a[0] == '@' && alen > 1) { a++; alen--; }
        if (username_len > 0 && alen == username_len &&
            strncasecmp(a, username, username_len) == 0)
            return true;
        if (user_id_len > 0 && alen == user_id_len &&
            memcmp(a, user_id, user_id_len) == 0)
            return true;
    }
    return false;
}
...
static sc_error_t build_get_updates_body(sc_allocator_t *alloc,
    int64_t offset, unsigned timeout_secs,
    char **out, size_t *out_len)
{
    sc_json_buf_t jbuf;
    sc_error_t err = sc_json_buf_init(&jbuf, alloc);
    if (err) return err;
    char num[128];
    int n = snprintf(num, sizeof(num), "{\"offset\":%lld,\"timeout\":%u,"
        "\"allowed_updates\":[\"message\"]}",
        (long long)offset, timeout_secs);
    if (n < 0 || (size_t)n >= sizeof(num)) { err = SC_ERR_INTERNAL; goto fail; }
    ...
    const char *chat_id, size_t chat_id_len)
{
    ...
    return;
#else
    if (!c->token || chat_id_len == 0) return;
    char url_buf[512];
    int n = build_api_url(url_buf, sizeof(url_buf), c->token, c->token_len,
        "sendChatAction");
    if (n < 0 || (size_t)n >= sizeof(url_buf)) return;
    char body_buf[256];
    n = snprintf(body_buf, sizeof(body_buf), "{\"chat_id\":%.*s,\"action\":\"typing\"}",
        (int)chat_id_len, chat_id);
    if (n < 0 || (size_t)n >= sizeof(body_buf)) return;
    sc_http_response_t resp = {0};
    sc_error_t err = sc_http_post_json(c->alloc, url_buf, NULL, body_buf,
        (size_t)n, &resp);
...
static const char *media_method(tg_media_kind_t k) {
    switch (k) {
        ...
        case TG_MEDIA_DOCUMENT: return "sendDocument";
        case TG_MEDIA_AUDIO:   return "sendAudio";
        case TG_MEDIA_VIDEO:   return "sendVideo";
    }
    return "sendDocument";
}
...
static const char *media_field(tg_media_kind_t k) {
    switch (k) {
        case TG_MEDIA_PHOTO:   return "photo";
        case TG_MEDIA_DOCUMENT: return "document";
        case TG_MEDIA_AUDIO:   return "audio";
        case TG_MEDIA_VIDEO:   return "video";
    }
    return "document";
}

static sc_error_t send_media_curl(sc_telegram_ctx_t *c,
    const char *chat_id, size_t chat_id_len,
    tg_media_kind_t kind,
    const char *media_path, size_t media_path_len,
    const char *caption, size_t caption_len)
{
#if SC_IS_TEST
    (void)c;
    (void)chat_id;
    ...
    return SC_OK;
#else
    if (!c->token) return SC_ERR_CHANNEL_NOT_CONFIGURED;
    char url_buf[512];
    int n = build_api_url(url_buf, sizeof(url_buf), c->token, c->token_len,
        media_method(kind));
    ...
    char chat_arg[160];
    n = snprintf(chat_arg, sizeof(chat_arg), "chat_id=%.*s",
        (int)chat_id_len, chat_id);
    if (n < 0 || (size_t)n >= sizeof(chat_arg)) return SC_ERR_INTERNAL;
    char file_arg[1100];
    ...
    if (is_url)
        ...
        n = snprintf(file_arg, sizeof(file_arg), "%s=@%.*s", field, (int)media_path_len, media_path);
    if (n < 0 || (size_t)n >= (int)sizeof(file_arg)) return SC_ERR_INTERNAL;
    const char *argv[16];
    int argc = 0;
    argv[argc++] = "curl";
    argv[argc++] = "-s";
    ...
    if (err != SC_OK) return SC_ERR_CHANNEL_SEND;
...
static tg_media_kind_t infer_media_kind(const char *path, size_t len) {
    ...
    if (!dot || dot >= path + len) return TG_MEDIA_DOCUMENT;
    ...
    if (ext_len >= 3) {
        ...
        if (memcmp(ext, "png", 3) == 0 || memcmp(ext, "jpg", 3) == 0 ||
            ...
            memcmp(ext, "webp", 4) == 0) return TG_MEDIA_PHOTO;
        if (memcmp(ext, "mp4", 3) == 0 || memcmp(ext, "mov", 3) == 0 ||
            memcmp(ext, "webm", 4) == 0) return TG_MEDIA_VIDEO;
...
static sc_error_t telegram_send(void *ctx,
    ...
    const char *const *media, size_t media_count)
{
    ...
    while (offset < message_len) {
        ...
        char *body = NULL;
        size_t body_len = 0;
        sc_error_t err = build_send_body(c->alloc,
            target, target_len,
...
static sc_error_t telegram_start_typing(void *ctx,
...
static const char *telegram_name(void *ctx) { (void)ctx; return "telegram"; }
...
static bool telegram_health_check(void *ctx) {
    ...
    if (n < 0 || (size_t)n >= sizeof(url_buf)) return false;
    sc_http_response_t resp = {0};
    sc_error_t err = sc_http_get(c->alloc, url_buf, NULL, &resp);
    bool ok = (err == SC_OK && resp.status_code == 200 &&
        resp.body && strstr(resp.body, "\"ok\":true") != NULL);
    if (resp.owned && resp.body)
        sc_http_response_free(c->alloc, &resp);
    return ok;
#endif
    ...
    size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    return SC_OK;
#else
    if (!c->token || c->token_len == 0) return SC_OK;
    ...
    char *body = NULL;
    size_t body_len = 0;
    sc_error_t err = build_get_updates_body(alloc, c->last_update_id, 25,
        &body, &body_len);
    ...
    for (size_t i = 0; i < result->data.array.len && cnt < max_msgs; i++) {
        ...
        if (!up || up->type != SC_JSON_OBJECT) continue;
        ...
        if (!msg_obj || msg_obj->type != SC_JSON_OBJECT) continue;

        sc_json_value_t *from = sc_json_object_get(msg_obj, "from");
        if (!from || from->type != SC_JSON_OBJECT) continue;
        ...
        if (!text) text = sc_json_get_string(msg_obj, "caption");
        if (!text) text = "";
        ...
        if (file_id) {
            ...
            int gu = build_api_url(getfile_url, sizeof(getfile_url), c->token, c->token_len, "getFile");
            if (gu > 0 && (size_t)gu < sizeof(getfile_url)) {
                ...
                if (gb > 0) {
                    ...
                    sc_error_t ger = sc_http_post_json(alloc, getfile_url, NULL, body_buf,
                        (size_t)gb, &gresp);
                    if (ger == SC_OK && gresp.body && gresp.body_len > 0) {
                        ...
                        if (sc_json_parse(alloc, gresp.body, gresp.body_len, &fp_parsed) == SC_OK
                            ...
                            const char *file_path = res && res->type == SC_JSON_OBJECT
                                ? sc_json_get_string(res, "file_path") : NULL;
                            if (file_path) {
                                char url_buf[768];
        ...
        cnt++;
        if (built) alloc->free(alloc->ctx, built, strlen(built) + 1);
    }
